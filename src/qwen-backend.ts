...
type QwenProcessor = Awaited<ReturnType<Transformers['AutoProcessor']['from_pretrained']>>
...
export class QwenVisionBackend extends VisionBackend {
  ...
  private async infer(request: VisionBackendRequest): Promise<string> {
    ...
    try {
      ...
      if (request.signal?.aborted) throw abortReason(request.signal)
      if (!('slice' in generated) || inputs.input_ids?.dims?.[1] === undefined) {
  ...
  private async inferGlm(request: VisionBackendRequest, config: ResolvedConfig): Promise<string> {
    ...
    const apiKey = process.env[config.apiKeyEnv]?.trim()
      ...
      || ''
    if (apiKey === '' && !local) {
      throw new Error(`GLM vision needs a key. Add the free Zhipu key in 插件配置 (credential ${config.apiKeyEnv}).`)
    }
    const models = config.baseURL === DEFAULT_GLM_BASE_URL && config.glmModel === DEFAULT_GLM_MODEL
    ...
    return withModelCacheLock(config.cacheDir, async () => {
      ...
      const options = {
        ...
        progress_callback: (progress: { status: string; progress?: number; loaded?: number; total?: number }) => {
          if (progress.status !== 'progress_total' || progress.progress === undefined) return
          const bucket = Math.floor(progress.progress / 10) * 10
          if (bucket <= progressBucket) return
          progressBucket = bucket
          this.logger.info('preparing local Qwen vision model: %d%% (%d / %d bytes)',
            bucket, progress.loaded ?? 0, progress.total ?? 0)
        },
      ...
      try {
        ...
        this.logger.warn('automatic accelerated model initialization failed; retrying on CPU: %s',
