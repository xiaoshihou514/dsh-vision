...
export async function discardCorruptModelFiles(
  root: string,
...
export async function verifyModelFiles(
  ...
  for (const file of files) {
    ...
    if (!(await exists(path)))
      throw new Error(`dsh-vision model download did not produce ${file.path}`);
    if ((await digest(path)) !== file.sha256)
      throw new Error(
        `dsh-vision model integrity check failed for ${file.path}`,
      );
  }
}

/** Serialize model cache mutation across Harness processes. */
export async function withModelCacheLock<T>(
  cacheDir: string,
  task: () => Promise<T>,
): Promise<T> {
...
export function modelRevisionRoot(
  cacheDir: string,
  modelId: string,
  revision: string,
): string {
