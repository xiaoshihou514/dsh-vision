...
async function safeImage(path, cwd) {
	...
	if (rel === ".." || rel.startsWith(`..${process.platform === "win32" ? "\\" : "/"}`) || isAbsolute(rel)) throw new Error(`view_image only reads images inside the current workspace: ${path}`);
	const mediaType = MEDIA_TYPES[extname(target).toLowerCase()];
	if (mediaType === void 0) throw new Error("view_image supports PNG, JPEG, WebP, and GIF files");
...
function apply(ctx) {
	ctx.tools.register(defineTool({
		...
		parameters: {
			...
			question: {
				type: "string",
				description: "Specific visual question; omit for a thorough description and OCR."
			}
		},
		output: {
			schema: { type: "string" },
