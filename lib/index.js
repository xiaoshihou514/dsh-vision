...
async function processorSource(config) {
...
var QwenVisionBackend = class extends VisionBackend {
  ...
  constructor(
    ctx,
    config,
    loadRuntime = () => import("@huggingface/transformers"),
  ...
  async infer(request) {
    ...
    try {
      ...
      if (request.signal?.aborted) throw abortReason(request.signal);
      if (!("slice" in generated) || inputs.input_ids?.dims?.[1] === void 0)
